...
static void RunSpeedTest(void)
{
	...
	now = SDL_GetTicks();
	mesg((char*)"Graphics speed test took %d microseconds per cycle.\r\n",
						((now-then)/test_reps));
}
...
void PrintUsage(void)
{
	...
	error((char*)"or\n");
	error((char*)"Usage: %s <options>\n\n", progname);
	error((char*)"Where <options> can be any of:\n\n"
...
int main(int argc, char *argv[])
{
	...
	for ( progname=argv[0]; --argc; ++argv ) {
		if ( strcmp(argv[1], (char*)"-fullscreen") == 0 ) {
			video_flags |= SDL_FULLSCREEN;
		} else
		if ( strcmp(argv[1], (char*)"-gamma") == 0 ) {
			int gammacorrect;

			if ( ! argv[2] ) {  /* Print the current gamma */
				mesg((char*)"Current Gamma correction level: %d\n",
								gGammaCorrect);
				exit(0);
			}
			if ( (gammacorrect=atoi(argv[2])) < 0 || 
							gammacorrect > 8 ) {
				error(
	(char*)"Gamma correction value must be between 0 and 8. -- Exiting.\n");
				exit(1);
			}
			...
			if ( ! argv[2] ) {  /* Print the current volume */
				mesg((char*)"Current volume level: %d\n",
								gSoundLevel);
				exit(0);
			}
			if ( (volume=atoi(argv[2])) < 0 || volume > 8 ) {
				error(
	(char*)"Volume must be a number between 0 and 8. -- Exiting.\n");
				exit(1);
			}
...
#define CHECKSUM_DEBUG
		...
		else if ( strcmp(argv[1], (char*)"-checksum") == 0 ) {
			mesg((char*)"Checksum = %s\n", get_checksum(NULL, 0));
			exit(0);
		}
#endif /* CHECKSUM_DEBUG */
		else if ( strcmp(argv[1], (char*)"-printscores") == 0 )
			doprinthigh = 1;
		else if ( strcmp(argv[1], (char*)"-netscores") == 0 )
			gNetScores = 1;
		else if ( strcmp(argv[1], (char*)"-speedtest") == 0 )
			speedtest = 1;
		else if ( LogicParseArgs(&argv, &argc) == 0 ) {
			/* LogicParseArgs() took care of everything */;
		} else if ( strcmp(argv[1], (char*)"-version") == 0 ) {
			error((char*)"%s", Version);
			exit(0);
		} else {
			PrintUsage();
		}
	...
	while ( gRunning ) {
		...
		if ( event.type == SDL_KEYDOWN ) {
			switch (event.key.keysym.sym) {
				...
				case SDLK_F3:
					screen->ScreenDump((char*)"ScoreDump",
							64, 48, 298, 384);
					break;
...
static void DrawSoundLevel(void)
{
	...
	if ( need_init ) {
		if ( (geneva = fontserv->NewFont((char*)"Geneva", 9)) == NULL ) {
			error((char*)"Can't use Geneva font! -- Exiting.\n");
			exit(255);
		}
	...
	sprintf(text, (char*)"%d", gSoundLevel);
	DrawText(xOff+309-7, yOff+240-6, text, geneva, STYLE_BOLD,
...
void DrawMainScreen(void)
{
	...
	if ( title == NULL ) {
		error((char*)"Can't load 'title' title! (ID=%d)\n", 129);
		exit(255);
        }
	...
	if ( (bigfont = fontserv->NewFont((char*)"New York", 18)) == NULL ) {
		error((char*)"Can't use New York (18) font! -- Exiting.\n");
		exit(255);
	}
	DrawText(xOff+5, botDiv+22, (char*)"Name", bigfont, STYLE_ULINE,
						0xFF, 0xFF, 0x00);
	sRt = xOff+185;
	DrawText(sRt, botDiv+22, (char*)"Score", bigfont, STYLE_ULINE,
						0xFF, 0xFF, 0x00);
	sRt += fontserv->TextWidth((char*)"Score", bigfont, STYLE_ULINE);
	wRt = xOff+245;
	DrawText(wRt, botDiv+22, (char*)"Wave", bigfont, STYLE_ULINE,
						0xFF, 0xFF, 0x00);
	wRt += fontserv->TextWidth((char*)"Wave", bigfont, STYLE_ULINE)-10;

	/* -- Now the scores */
	LoadScores();
	if ( (font = fontserv->NewFont((char*)"New York", 14)) == NULL ) {
		error((char*)"Can't use New York (14) font! -- Exiting.\n");
		exit(255);
	}
	...
	DrawText(xOff+5, botDiv+46+(10*18)+3, (char*)"Last Score: ", 
					bigfont, STYLE_NORM, 0xFF, 0xFF, 0xFF);
	sprintf(buffer, (char*)"%d", GetScore());
	sw = fontserv->TextWidth("Last Score: (char*)", bigfont, STYLE_NORM);
	...
	pt.h = rightDiv + 10;
	pt.v = yOff + 10;
	DrawKey(&pt, (char*)"P", (char*)" Start playing Maelstrom", RunPlayGame);

	pt.h = rightDiv + 10;
	pt.v += offset;
	DrawKey(&pt, (char*)"C", (char*)" Configure the game controls", RunConfigureControls);

	pt.h = rightDiv + 10;
	pt.v += offset;
	DrawKey(&pt, (char*)"Z", (char*)" Zap the high scores", RunZapScores);

	pt.h = rightDiv + 10;
	pt.v += offset;
	DrawKey(&pt, (char*)"A", (char*)" About Maelstrom...", RunDoAbout);

	pt.v += offset;

	pt.h = rightDiv + 10;
	pt.v += offset;
	DrawKey(&pt, (char*)"Q", (char*)" Quit Maelstrom", RunQuitGame);

	pt.h = rightDiv + 10;
	pt.v += offset;
	DrawKey(&pt, (char*)"0", (char*)" (char*)", DecrementSound);

	if ( (font = fontserv->NewFont("Geneva", 9)) == NULL ) {
		error((char*)"Can't use Geneva font! -- Exiting.\n");
		exit(255);
	}
	DrawText(pt.h+gKeyIcon->w+3, pt.v+19, (char*)"-",
				font, STYLE_NORM, 0xFF, 0xFF, 0x00);

	pt.h = rightDiv + 50;
	DrawKey(&pt, (char*)"8", (char*)" Set Sound Volume", IncrementSound);

/* -- Draw the credits */

	DrawText(xOff+5+68, yOff+5+127, (char*)"Port to Linux by Sam Lantinga",
				font, STYLE_BOLD, 0xFF, 0xFF, 0x00);
	DrawText(rightDiv+10, yOff+259, (char*)"©1992-4 Ambrosia Software, Inc.",
				font, STYLE_BOLD, 0xFF, 0xFF, 0xFF);

/* -- Draw the version number */

	DrawText(xOff+20, yOff+151, (char*)VERSION_STRING,
